...
Aplicamos un último cambio. Dado a que habíamos encontrado datos de juegos catalogados como Free 2 Play a pesar de tener precio mayor que cero, y a que el rango de precios tiene una alta concentración en valores bajos, decidimos dejar una sola columna que reemplaza el valor de todos los juegos pagos por un 1. Esto nos facilitará el explorar los datos en mayor profuncidad.
...
use_python("/usr/bin/python83")
```
```{r}
steam_nuevo <- read.csv("data/steam_ready.csv", encoding = "UTF-8", as.is = F)
```
```{python}
import matplotlib.pyplot as plt 
steam_py = r.steam_nuevo
plt.scatter(steam_py[:,7],steam_py[:,8])
plt.title("Scatterplot: dueños promedio vs ranking neto")
plt.show
```
